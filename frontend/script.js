...
function displayResult(prediction, confidence) {
    ...
    // Optional confidence text, e.g. " (confidence: 97%)"
...
/*
    Flask backend endpoint (backend/app.py).

    Request:   POST  {"email": "<message text>"}
...
async function checkEmailWithBackend(email) {
    ...
    if (!response.ok) {
        ...
        // The Flask backend replies with {"error": "..."} for bad input

        let backendMessage = `Backend request failed (${response.status}).`;
...
spamForm.addEventListener("submit", async function (event) {
    ...
    try {
        ...
        */
        ...
        displayResult(prediction, data.confidence);
